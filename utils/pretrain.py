...
def pretrain(model, X_train, X_test, epochs, batch_size, opt, name, fn, save):
    ...
    for i in range(1, len(model.layers)):
        ...
        embedder = Model(model.get_layer(index=0).input, model.get_layer(index=i-1).output)
        data = embedder.predict(X_train)

        noise = tf.random.normal(shape=data.shape, mean=0.0, stddev=1, dtype=tf.float32).numpy()
